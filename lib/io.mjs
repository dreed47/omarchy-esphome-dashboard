...
import {
    openSync, closeSync, readSync, writeSync, fstatSync, mkdirSync,
    chmodSync, unlinkSync, renameSync, constants as fsConstants,
} from "node:fs"
...
// Reads/writes below never follow a symlink at HA_CONFIG_PATH (O_NOFOLLOW):
// this file holds the HA bearer token, and something else able to write
// into our own config directory could otherwise plant a symlink there to
// have us read from or clobber an arbitrary file this user owns.
export function readHaConfig() {
    ...
    try {
        ...
        // Not a regular file, not ours, or suspiciously large: treat as
        // absent rather than trusting it.
        if (!st.isFile() || st.uid !== process.getuid() || st.size > MAX_HA_CONFIG_BYTES) return null
...
// by anyone else), then rename it into place. A rename replaces whatever
// directory entry is at HA_CONFIG_PATH - including a symlink - rather than
// following it, so this can't be tricked into writing through one either.
